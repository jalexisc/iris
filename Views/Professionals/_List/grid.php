...
/**
* █ ---------------------------------------------------------------------------------------------------------------------
* █ ░FRAMEWORK                                  2025-04-10 06:51:46
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Professionals\List\table.php]
...
**/
//[models]--------------------------------------------------------------------------------------------------------------
$mprofessionals = model('App\Modules\Iris\Models\Iris_Professionals');
//[vars]----------------------------------------------------------------------------------------------------------------
$back= "/iris";
...
$fields = array(
		 //"professional" => lang("App.professional"),
		 //"username" => lang("App.username"),
...
//[build]--------------------------------------------------------------------------------------------------------------
$conditions = array();
//$mprofessionals->clear_AllCache();
$rows = $mprofessionals->get_CachedSearch($conditions,$limit, $offset,"professional DESC");
$total = $mprofessionals->get_CountAllResults($conditions);
...
$bgrid->set_Headers(array(
		array("content" => "#", "class" => "text-center	align-middle"),
		 //array("content" => lang("App.professional"), "class" => "text-center	align-middle"),
		 //array("content" => lang("App.username"), "class" => "text-center	align-middle"),
		 ...
		 //array("content" => lang("App.password"), "class" => "text-center	align-middle"),
...
$component = '/iris/professionals';
$count = $offset;
foreach ($rows["data"] as $row) {
		if(!empty($row["professional"])){
				$count++;
				//[links]-------------------------------------------------------------------------------------------------------
				$hrefView="$component/view/{$row["professional"]}";
				$hrefEdit="$component/edit/{$row["professional"]}";
				$hrefDelete="$component/delete/{$row["professional"]}";
				//[buttons]-----------------------------------------------------------------------------------------------------
				...
				$bgrid->add_Row(
						array(
								array("content" => $count, "class" => "text-center align-middle"),
								 //array("content" => $row['professional'], "class" => "text-left align-middle"),
								 //array("content" => $row['username'], "class" => "text-left align-middle"),
								 //array("content" => $row['specialty'], "class" => "text-left align-middle"),
								 //array("content" => $row['email'], "class" => "text-left align-middle"),
								 //array("content" => $row['password'], "class" => "text-left align-middle"),
...
$card = $bootstrap->get_Card("card-grid", array(
		"title" =>lang('Professionals.list-title'),
		"header-back" => $back,
		"header-add"=>"/sie/q10files/import/" . lpk(),
		"alert" => array("icon" => ICON_INFO, "type" => "info", "title" => lang('Professionals.list-title'), "message" => lang('Professionals.list-description')),
		"content" => $bgrid,
));
echo($card);
?>
