...
* █ ░█▀▀█ █▀▀█ █▀▀▄ █▀▀ ░█─░█ ─▀─ █▀▀▀ █▀▀▀ █▀▀ [App\Modules\Iris\Views\Patients\Delete\form.php]
...
$r= $model->getPatient($oid);
$name = urldecode($r["name"]);
$message=sprintf(lang("Iris_Patients.delete-message"),$name);
...
$card = $bootstrap->get_Card("delete-{$oid}", array(
		 "class" =>"card-info",
		 "icon" => ICON_WARNING,
		 "title" => sprintf(lang("Iris_Patients.delete-title"),$name),
		 "text-class" =>"text-center",
		 "text"=>sprintf(lang("Iris_Patients.delete-message"),$name),
		 "content" =>$f,
		 "header-back" =>$back
));
echo($card);
