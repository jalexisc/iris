...
/**
* @Copilot Siempre que en el código se cree una variable llamada $mepisodes, esta deberá ser igualada a  model('App\Modules\Iris\Models\Iris_Episodes');
* @Instruction $mepisodes = model('App\Modules\Iris\Models\Iris_Episodes');
...
/**
		...
		public function get_List(int $limit, int $offset, string $search = ""): array|false
		{
				$result = $this
						->groupStart()
						->like("episode", "%{$search}%")
						->orLike("patient", "%{$search}%")
						->orLike("start_date", "%{$search}%")
						->orLike("end_date", "%{$search}%")
						->orLike("reason_for_visit", "%{$search}%")
						->orLike("general_notes", "%{$search}%")
